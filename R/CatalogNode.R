...
#'  \donttest{
#'   library(thredds)
#'   top_uri <- 'https://psl.noaa.gov/thredds/catalog/Datasets/godas/catalog.xml'
#'   Top <- thredds::CatalogNode$new(top_uri)
#'   
#'   #to browse catalogue
#'   #Top$browse()
#'   
#'   #go down in 'Derived' catalog
#'   cat_name = "Derived"
#'   subcat <- Top$get_catalogs(cat_name)[[cat_name]]
#'   
#'   #get dataset node
#'   ds_name = "salt.mon.ltm.1991-2020.nc"
#'   ds <- subcat$get_datasets(ds_name)[[1]]
#'   
...
#'   #if(require("ncdf4")){
#'   #  base_uri = paste0(
#'   #   "https://", gsub("^(https?://)?([^/]+).*", "\\\2", subcat$url), 
#'   #   subcat$list_services()$odap[["base"]]
#'   #  )
